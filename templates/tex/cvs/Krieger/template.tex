...
\highlightbar{
    ...
    {% if profile.website_active  %}\github{ {{- profile.get_tex_value("website")  -}} }{% endif %}
...
\mainbar{
    ...
    {% if profile.publication_active and profile.publication_set.count() > 0 %}
      \section[\faLightbulb]{ {{- profile.publication_label| latex_escape  -}} }
    ...
    {% if profile.volunteering_active and profile.volunteering_set.count() > 0 %}
      \section[\faGears]{ {{- profile.volunteering_label| latex_escape  -}} }
      {% for volunteering in profile.volunteering_set.all() %}
